...
app.post('/api/user/:id/config', authenticateToken, authorizeAdmin, async (req, res) => {
    try {
        const id = req.params.id;
...
app.post('/api/user/:id/reset', authenticateToken, authorizeAdmin, async (req, res) => {
    try {
        const id = req.params.id;
        const update = {
            $set: {
...
app.get('/api/userme', authenticateToken, async (req, res) => {
...
app.post('/api/usergenerate-wallet', authenticateToken, async (req, res) => {
    try {
...
app.post('/api/uservalidate-wallet-preview', authenticateToken, async (req, res) => {
    try {
...
app.post('/api/userimport-wallet', authenticateToken, async (req, res) => {
    try {
        let { privateKey, proxyAddress } = req.body;
        if (!privateKey)
            return res.status(400).json({ error: 'Private key required' });
...
app.post('/api/userupdate-config', authenticateToken, async (req, res) => {
    const user = await User.findById(req.user?.id);
    if (!user)
        return res.status(404).json({ error: 'User not found' });
...
app.get('/api/userpositions', authenticateToken, async (req, res) => {
    try {
        const user = req.fullUser;
...
app.get('/api/usertrades', authenticateToken, async (req, res) => {
    try {
        const { Activity } = await import('../models/userHistory.js');
        const user = req.fullUser;
        ...
        const enriched = await Promise.all(tradesData.map(async (t) => {
            ...
            let myEntryAmount = userStatus?.myEntryAmount || null;
            let myEntryPrice = userStatus?.myEntryPrice || null;
